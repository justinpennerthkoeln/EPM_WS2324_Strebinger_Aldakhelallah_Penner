document.addEventListener("DOMContentLoaded", () => {
	...
	if (setting == undefined) {
		document
			.querySelector("aside#rename-collection")
	...
	renameButton.addEventListener("click", ($event) => {
		...
		if (renameInput.length > 0) {
			fetch(
				`/api/collections/${window.location.pathname.split("/")[2]}/rename`,
				{
					method: "POST",
					headers: {
	...
	// Delete Collection
	const deleteForm = document.querySelector("#delete-collection form");
	...
	descriptionForm.addEventListener("submit", ($event) => {
		...
		const descriptionInput = document.querySelector(
			"#update-description form textarea"
		);
	...
	const inviteCollaborators = Vue.createApp({
		data() {
			return {
				searchResults: [],
				searchInput: "",
		...
		template: `
            <input type="text" class="search-user" placeholder="Enter a username..." v-model="searchInput" @input="searchUsers" />
            <ul :class="{disabled: searchResults.length == 0}">
                <li v-for="user in searchResults" :key="user.id" @click="selectUser(user.id)" :class="{selected: selected == user.id}">@{{user.username}}</li>
            </ul>
            <button :disabled="!selected" @click="inviteUser(selected)">Invite collaborator</button>
        `,
		methods: {
			searchUsers() {
				this.selected = null;
				...
				if (this.searchInput.length > 0) {
					fetch(`/api/users?searchTerm=${this.searchInput.toLowerCase()}`, {
						method: "GET",
						headers: {
							"Content-Type": "application/json",
						},
			...
			selectUser(userId) {
				this.selected = this.selected == userId ? null : userId;
			},

			inviteUser(userId) {
				if (userId != null) {
					if (userId == JSON.parse(localStorage.getItem("user")).id) {
						window.location = `${window.location.origin}${window.location.pathname}?error=You can't invite yourself`;
					} else {
						fetch(
							`/api/collections/${
								window.location.pathname.split("/")[2]
	...
	const manageCollaborators = Vue.createApp({
		...
		template: `
            <ul>
                <li v-for="collaborator in collaborators" class="collaborator">
                    <p>@{{collaborator.username}}</p>
                    <form @submit.prevent>
                        <button class="delete-collaborator" :value="collaborator.membership_id" @click="deleteCollaborator(collaborator.membership_id, collaborator.username)">Delete collaborator</button>
                    </form>
                </li>
            </ul>
        `,
		methods: {
			...
			deleteCollaborator(membershipId, username) {
				if (
	...
	// Manage projects
	const manageProjects = Vue.createApp({
		data() {
			return {
				platforms: [],
				possiblePlatforms: ["GitHub", "GitLab", "Dribbble", "Figma", "Notion"],
			};
		},
		template: `
            <ul>
                <li v-for="platform in platforms">
                    <p>{{findMatchingPlatform(platform.platform)}}</p>
                    <form @submit.prevent>
                        <input type="text" name="target-document" v-model="platform.target_document" :value="platform.target_document" id="target-document" class="target-document">
                        <div>
                            <button class="update-platform" :value="platform.platform_id" @click="updatePlatform(platform.platform_id, platform.target_document)">Update project</button>
                            <button class="delete-platform" :value="platform.platform_id" @click="deletePlatform(platform.platform_id)">Delete project</button>
                        </div>                    
                    </form>
                </li>
            </ul>
		...
		methods: {
			...
			findMatchingPlatform(platform) {
				return this.possiblePlatforms.find(
					(possiblePlatform) =>
						possiblePlatform.toLowerCase() == platform.toLowerCase()
				);
			},
			...
			deletePlatform(platformId) {
				...
				Promise.resolve(platform).then((platform) => {
					removeWebhook(platform, window.location.pathname.split("/")[2]);
			...
			updatePlatform(platformId, targetDocument) {
				...
				fetch(
					`/api/platforms/${platformId}/target-document?document=${targetDocument}`,
					{
						method: "POST",
						headers: {
							"Content-Type": "application/x-www-form-urlencoded",
	...
	const alerts = Vue.createApp({
		...
		template: `
            <ul>
                <li v-for="setting in setting_options" class="alert-template" @click="updateSetting(setting.alert_settings_id, !setting.value, setting.setting)">
                    <p>{{setting.setting}}</p>
                    <div class="switch">
                        <input
                            type="checkbox"
                            ...
                            @change="updateSetting(setting.alert_settings_id, setting.value, setting.setting)"
                        />
		...
		methods: {
			getAlerts() {
				fetch(
					`/api/alerts/${window.location.pathname.split("/")[2]}/settings`,
					{
						method: "GET",
						headers: {
							"Content-Type": "application/json",
...
// Webhooks
async function removeWebhook(repo, uuid) {
	if(repo.platform == "figma" && repo.team_id != null) {
		...
		}).then((data) => {window.location = `${window.location.origin}${window.location.pathname}?success=Successfully deleted project`;});
...
async function connectGitlab(uuid, platformId) {
	window.location.href = `https://gitlab.com/oauth/authorize?client_id=ee480e58dacb20b4af3ea2eada267495191ea86740dde4360149d42a9b2706ac&redirect_uri=http://localhost:80/oauth/gitlab?ids=${
		uuid + "_" + platformId
	}&response_type=code&state=STATE&scopes=write_repository,read_user`;
}

async function connectDribbble(uuid, platformId) {
	window.location.href = `https://dribbble.com/oauth/authorize?client_id=40c594e9554be586ed8cffafe32c3ab44b3b62d16aecb00a8a68a52b3430d358&redirect_uri=http://localhost:80/oauth/dribbble&scope=public+write&state=${
		uuid + "_" + platformId
	}`;
}
...
async function connectNotion(uuid, platformId) {
	...
	window.location.href = `https://api.notion.com/v1/oauth/authorize?response_type=code&client_id=40fc3257-b3fe-4337-ab8a-297c5a970609&owner=user&redirect_uri=${redirect_uri}&state=${
		uuid + "_" + platformId
	}`;
}
