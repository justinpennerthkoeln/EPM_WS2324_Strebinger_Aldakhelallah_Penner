const params = new URLSearchParams(window.location.search);
    ...
    function setToInput(platform) {
        ...
        form.addEventListener("submit", ($event) => {
            ...
            fetch(`/api/platforms/${params.get("platformId")}/target-document?document=${getTargetFromLink(input.value, params.get("platform")).toString()}`, {
                method: "POST",
                headers: {
            ...
            window.location = `${window.location.origin}/collection/${params.get("uuid")}/settings/add-projects?success=Added Page`;
        });
    }
    ...
    async function loadRepos(platform, platformId) {
        ...
        switch (platformData.platform) {
            case "github":
                ...
                break;
            case "gitlab":
                var repos = await fetch(`https://gitlab.com/api/v4/projects?membership=true`, {
                    method: 'GET',
                    headers: {
        ...
        repos.forEach(repo => {
            ...
            REPO.querySelector(".select-repo-btn").addEventListener("click", ($event) => {
                ...
                if(platform === "github") {
                    ...
                    fetch(`/api/platforms/${params.get("platformId")}/target-document?document=${repo.id}`, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                            "Accept": "application/json"
    ...
    async function loadShots(platformId) {
        ...
        var shots = fetch(`https://api.dribbble.com/v2/user/shots?access_token=${platformData.platform_key}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json'
    ...
    async function generateShots(shots) {
        ...
        shots.forEach(shot => {
            ...
            REPO.querySelector(".select-repo-btn").addEventListener("click", ($event) => {
                $event.preventDefault();
                fetch(`/api/platforms/${params.get("platformId")}/target-document?document=${shot.id}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Accept": "application/json"
                ...
                window.location = `${window.location.origin}/collection/${params.get("uuid")}/settings/add-projects?success=Added Shot`;
            });
            REPOS.appendChild(REPO);
        });
    }